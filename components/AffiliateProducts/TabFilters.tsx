...
import MySwitch from "../shared/MySwitch";
...
const TabFilters = () => {
  ...
  const handleChangeAge = (checked: boolean, name: string) => {
    checked
      ? setAgeState([...ageState, name])
      : setAgeState(ageState.filter((i) => i !== name));
  };
          ...
          <>
            ...
            >
              ...
              {!ageState.length ? (
                ...
                <span onClick={() => setAgeState([])}>{renderXClear()}</span>
              )}
            </PopoverButton>
            <Transition
              as={Fragment}
                    ...
                    {DATA_age.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox
                          name={item.name}
                          label={item.name}
                          defaultChecked={ageState.includes(item.name)}
                          onChange={(checked) =>
                            handleChangeAge(checked, item.name)
                          }
                        />
                      </div>
                    ))}
